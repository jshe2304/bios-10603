...
proc sel_sel_angle_frames { mol1 mol2 sel1text sel2text } {
  set outfile [open "angles.dat" w+]

  set N [molinfo $mol1 get numframes]
  set sel1 [atomselect $mol1 $sel1text]
  set sel2 [atomselect $mol2 $sel2text]

  for { set i 0 } {$i < $N} {incr i} {
    $sel1 frame $i
    $sel2 frame $i

    puts $outfile "$i \t [sel_sel_angle $sel1 $sel2]"
  }
  close $outfile
}
...
proc sel_angle_frames_file { mol seltext vec } {
  set outfile [open "angles.dat" w+]
  ...
  for { set i 0 } {$i < $N} {incr i} {
    ...
    puts $outfile "$i \t [sel_angle $sel $vec]"
  }
  $sel delete
  close $outfile
}
